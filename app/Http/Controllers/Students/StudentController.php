...
//        $data['my_sections'] = Section::all();
    ...
    public function updateProblems(Request $request)
    {
        ...
        /*
                ...
                $levels = Level::all();
                $problims = Problem::all();


                $histories = History::all();
                foreach ($histories as $history) {
                    return view('pages.Students.index', compact('students', 'history','levels','problims'));
                }
        */
    ...
    public function importExcel(Request $request)
    {
        ...
        $problims  = Problem::all();
        $students = Student::all();
        return view('pages.Students.index', compact('students','levels','problims'));
    }
    ...
    public function getSearch(Request $request)
    {
        ...
        if ($request->levels) {
            ...
            $students = Student::where('level_id', $request->levels)->get();
            $problims = Problem::all();
            $levels = Level::all();
            $histories = History::all();
            foreach ($histories as $history) {
                return view('pages.Students.index', compact('students', 'history', 'problims', 'levels'));
            }

        } elseif ($request->problims) {

//            dd($request->problims);

            $students = Student::WhenProblemId($request->problims)->get();
    ...
    public function StudentProblemsDelete(Request $request)
    {
//        $problims = Problem::whereId($id)->get();
//        dd($request->all());
        $id = $request->student_id;
        $students = Student::whereId($id)->get();
        foreach ($students as $student){
            $student->problems()->detach($request->problem_id);
